...
function compile(
    ...
    parent::Union{AExpr,Nothing} = nothing,
)::Expr
...
function compileassign(expr::AExpr, data::Dict{String,Any}, parent::Union{AExpr,Nothing})
    ...
    if (typeof(expr.args[2]) == AExpr && expr.args[2].head == :fn) # handle function assignments
        if type !== nothing # handle function with typed arguments/return type
            ...
            quote
                function $(compile(expr.args[1], data))(
                    $(typedargexprs...),
                )::$(compile(type.args[end], data))
            ...
            quote
                function $(compile(expr.args[1], data))(
                    $(compile(expr.args[2].args[1], data).args...),
                )
                    $(compile(expr.args[2].args[2], data))
                end
            end
...
function compiletypealias(expr::AExpr, data::Dict{String,Any})
    name = expr.args[1]
    fields = map(field -> (:($(field.args[1])::$(field.args[2]))), expr.args[2].args)
    quote
        struct $(name)
            $(fields...)
        end
...
function compilecall(expr::AExpr, data::Dict{String,Any})
    ...
    elseif fnName == :prev
        if expr.args[2] == :obj
            ...
            :($(Symbol(string(expr.args[2]) * "Prev"))(
                $(map(compile, expr.args[3:end])...),
            ))
        end
    elseif fnName != :(==)
...
function compilecase(expr::AExpr, data::Dict{String,Any})
    quote
        @match $(compile(expr.args[1], data)) begin
            $(
                map(
                    x -> :($(compile(x.args[1], data)) => $(compile(x.args[2], data))),
                    expr.args[2:end],
                )...
            )
        end
...
function compileobject(expr::AExpr, data::Dict{String,Any})
    ...
    custom_fields = map(
        field -> (:($(field.args[1])::$(compile(field.args[2], data)))),
    ...
    rendering = compile(
        filter(x -> (typeof(x) != AExpr) || (x.head != :typedecl), expr.args[2:end])[1],
        data,
    )
    quote
        ...
        function $(name)($(vcat(custom_fields, :(origin::Position))...))::$(name)
            ...
            rendering = $(rendering)
            $(name)(
                state.objectsCreated,
                origin,
                true,
                false,
                $(custom_field_names...),
                rendering isa AbstractArray ? vcat(rendering...) : [rendering],
            )
        end
    end
end
...
function compileinitnext(data::Dict{String,Any})
    ...
    list_objects = filter(
        x -> get(data["types"], x, :Any) in map(x -> [:List, x], data["objects"]),
    ...
    init = quote
        ...
        $(
            map(
                x -> :($(compile(x.args[1], data)) =
                        $(compile(x.args[2].args[1], data))),
                data["initnext"],
            )...
        )
        $(
            map(
                x -> :(foreach(x -> x.changed = false, $(compile(x, data)))),
                list_objects,
            )...
        )
    end
    ...
    notOnClausesConstant = map(
                            ...
                            filter(
                                z ->
                                    (
                                        ...
                                        (z[2].head == :let) && (
                                            x.args[1] in map(w -> w.args[1], z[2].args)
                                        )
                                    ),
                                data["on"],
                            ),
        ...
        filter(
            x -> !(
                get(data["types"], x.args[1], :Any) in
                map(y -> [:List, y], data["objects"])
            ),
            data["initnext"],
        ),
    )

    notOnClausesList = map(
        x -> quote
            $(Meta.parse(string(compile(x.args[1], data), "Changed"))) =
                filter(o -> o.changed, $(compile(x.args[1], data)))
            $(compile(x.args[1], data)) = filter(
                o -> !(
                    o.id in map(
                        x -> x.id,
                        $(Meta.parse(string(compile(x.args[1], data), "Changed"))),
                ...
                $(compile(x.args[2].args[2], data)),
            )
            $(compile(x.args[1], data)) = vcat(
                $(Meta.parse(string(compile(x.args[1], data), "Changed")))...,
                $(compile(x.args[1], data))...,
            )
            $(compile(x.args[1], data)) =
                filter(o -> o.alive, $(compile(x.args[1], data)))
            foreach(o -> o.changed = false, $(compile(x.args[1], data)))
        end,
        filter(
            x -> get(data["types"], x.args[1], :Any) in
            map(y -> [:List, y], data["objects"]),
            data["initnext"],
        ),
    )
    ...
    next = quote
        $(
            map(
                x -> :(
                    $(compile(x.args[1], data)) =
                        state.$(Symbol(string(x.args[1]) * "History"))[state.time-1]
                ),
        ...
        $(
            map(
                x -> :($(compile(x.args[1], data)) = $(compile(x.args[2], data))),
                filter(x -> x.args[1] != :GRID_SIZE, data["lifted"]),
            )...
        )
    end
        ...
        function init(
            $(
                map(
                    x -> :(
                        $(x.args[1])::Union{
                            $(compile(data["types"][x.args[1]], data)),
                            Nothing,
                        }
                    ),
                    data["external"],
            ...
            $(
                map(
                    x -> :(
                        state.$(Symbol(string(x.args[1]) * "History"))[state.time] =
                            $(x.args[1])
                    ),
            ...
            state.scene = Scene(
                vcat(
                    [
                        $(
                            filter(
                                x -> get(data["types"], x, :Any) in vcat(
                                    data["objects"],
                                    map(x -> [:List, x], data["objects"]),
                                ),
                                map(x -> x.args[1], vcat(data["initnext"], data["lifted"])),
                            )...
                        ),
                ...
                if :backgroundHistory in fieldnames(STATE)
...
function compilestatestruct(data::Dict{String,Any})
    ...
    stateParamsExternal = map(
        expr -> :(
            $(Symbol(
                string(expr.args[1]) * "History",
            ))::Dict{
                Int64,
                Union{$(compile(data["types"][expr.args[1]], data)),Nothing},
            }
        ),
        data["external"],
    )
...
function compileinitstate(data::Dict{String,Any})
    ...
    initStateParams =
        [0, 0, :(Scene([])), initStateParamsInternal..., initStateParamsExternal...]
...
function compileprevfuncs(data::Dict{String,Any})
    prevFunctions = map(
        x -> quote
            function $(Symbol(string(x.args[1]) * "Prev"))(
                n::Int = 1,
            )::$(
                if haskey(data["types"], x.args[1])
                    compile(data["types"][x.args[1]], data)
                else
                    Any
                end
            )
                state.$(Symbol(string(x.args[1]) * "History"))[if state.time - n >= 0
                    state.time - n
                else
                    0
                end]
    ...
    prevFunctions = vcat(
        ...
        map(
            x -> quote
                function $(Symbol(string(x.args[1]) * "Prev"))(
                    n::Int = 1,
                )::Union{$(compile(data["types"][x.args[1]], data)),Nothing}
                    state.$(Symbol(string(x.args[1]) * "History"))[if state.time - n >= 0
                        state.time - n
                    else
                        0
...
function compilebuiltin()
    ...
    [
        occurredFunction,
        utils,
        uniformChoiceFunction,
        uniformChoiceFunction2,
        minFunction,
        rangeFunction,
    ]
end
    ...
    "utils"          => quote
        ...
        function nextLiquid(object::Object)::Object
            ...
            if object.origin.y != GRID_SIZE_Y - 1 && isFree(move(object.origin, Position(0, 1)))
                ...
                leftHoles = filter(pos -> (pos.y == object.origin.y + 1) && (pos.x < object.origin.x) && isFree(pos), allPositions())
                rightHoles = filter(pos -> (pos.y == object.origin.y + 1) && (pos.x > object.origin.x) && isFree(pos), allPositions())
        ...
        function isFree(position::Position, object::Object)
            length(filter(cell -> cell.position.x == position.x && cell.position.y == position.y, render(Scene(filter(obj -> obj.id != object.id, state.scene.objects), state.scene.background)))) == 0
        end
