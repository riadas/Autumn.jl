...
function compiletojulia(aexpr::AExpr)::Expr
    ...
    historydata = Dict([
        (
            "external" => [
                au"""(external (: click Click))""".args[1],
                au"""(external (: left KeyPress))""".args[1],
                au"""(external (: right KeyPress))""".args[1],
                au"""(external (: up KeyPress))""".args[1],
                au"""(external (: down KeyPress))""".args[1],
            ]
        ), # :typedecl aexprs for all external variables
        ...
        ), # map of global variable names (symbols) to types
        ("on" => []),
        ("objects" => []),
    ])
    ...
    if (aexpr.head == :program)
        ...
        lines =
            filter(x -> x !== :(), map(arg -> compile(arg, historydata, aexpr), aexpr.args))
