...
export empty_env,
    ...
    interpret_program,
    interpret_over_time,
    interpret_over_time_observations,
    interpret_over_time_observations_and_env
import MLStyle
...
function start(aex::AExpr, rng = Random.GLOBAL_RNG; show_rules = -1)
    ...
    env = Env(
        ...
        Dict(),
        Dict(),
        State(0, 0, rng, Scene([], "white"), Dict(), Dict()),
        show_rules,
    )
    ...
    initnext_lines = filter(
        l -> l.head == :assign && (l.args[2] isa AExpr && l.args[2].head == :initnext),
        lines,
    )
    lifted_lines = filter(
        l ->
            l.head == :assign &&
                (!(l.args[2] isa AExpr) || l.args[2].head != :initnext),
        lines,
    ) # GRID_SIZE and background here
    on_clause_lines = filter(l -> l.head == :on, lines)

    reordered_lines_temp = vcat(
        grid_params_and_object_type_lines,
        initnext_lines,
        on_clause_lines,
        lifted_lines,
    )

    reordered_lines = vcat(
        grid_params_and_object_type_lines,
        on_clause_lines,
        initnext_lines,
        lifted_lines,
    ...
    for line in lifted_lines
        ...
        _, env = interpret(
            AExpr(
                ...
                parseautumn(
                    """(fn () (get (.. (.. state histories) $(string(var_name))) (- (.. state time) 1) $(var_name)))""",
                ),
            ),
            env,
        )
    ...
    background =
        background_assignments != [] ? background_assignments[end].args[2] : "#ffffff00"
...
function step(
    aex::AExpr,
    env::Env,
    user_events = (click = nothing, left = false, right = false, down = false, up = false),
)::Env
...
function update_state(env_::Env)
    ...
    for key in keys(env_.current_var_values)
        if (
            (env_.current_var_values[key] isa Object) || (
                env_.current_var_values[key] isa AbstractArray &&
                (length(env_.current_var_values[key]) > 0) &&
                (env_.current_var_values[key][1] isa Object)
            )
        )
...
function interpret_over_time_observations(
    aex::AExpr,
    iters,
    user_events = [],
...
function interpret_over_time_observations_and_env(
    aex::AExpr,
    iters,
    user_events = [],
    rng = Random.GLOBAL_RNG,
