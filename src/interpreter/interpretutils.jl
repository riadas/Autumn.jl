...
lib_to_func = Dict(
    :Position => AutumnStandardLibrary.Position,
    ...
    elseif f == :clicked
        ...
        for arg in args
            if (arg isa AbstractArray) &&
               (length(arg) == 2) &&
               (arg[1] isa AExpr || arg[1] isa Symbol) &&
               (arg[2] isa AExpr || arg[2] isa Symbol)
...
function interpret(aex::AExpr, @nospecialize(Γ::Env))
    ...
    t = MLStyle.@match arr begin
        ...
        [:assign, x, v::AExpr] && if v.head == :initnext
        end => interpret_init_next(x, v, Γ)
        ...
        [:lambda, args...]                               => (args, Γ)
        [:fn, args...]                                   => (args, Γ)
        [:call, f, arg1] && if isprim(f)
        end                 => let (new_arg, Γ2) = interpret(arg1, Γ)
            primapl(f, new_arg, Γ2)
        end

        [:call, f, arg1, arg2] && if isprim(f)
        end                    => let (new_arg1, Γ2) = interpret(arg1, Γ)
            (new_arg2, Γ2) = interpret(arg2, Γ2)
            primapl(f, new_arg1, new_arg2, Γ2)
        end
        [:call, f, args...] && if f == :prev && args != [:obj]
        end    => interpret(AExpr(:call, Symbol(string(f, uppercasefirst(string(args[1])))), :state), Γ)
        [:call, f, args...] && if islib(f)
        end                        => interpret_lib(f, args, Γ)
        [:call, f, args...] && if isjulialib(f)
        end                   => interpret_julia_lib(f, args, Γ)
        [:call, f, args...] && if f in keys(Γ.state.object_types)
        end => interpret_object_call(f, args, Γ)
...
function interpret_object_call(f, args, @nospecialize(Γ::Env))
    ...
    if length(fields) == 0
        object_repr =
            Object(Γ.state.objectsCreated, origin, f, true, false, field_values, nothing)
    else
        render, Γ2 = interpret(Γ.state.object_types[f].render, Γ2)
        render = render isa AbstractArray ? render : [render]
        object_repr =
            Object(Γ.state.objectsCreated, origin, f, true, false, field_values, render)
    end
    Γ.current_var_values = old_current_var_values
    # # # # # # println("AFTER")
    # # # # # # @showΓ.state.objectsCreated 
...
function interpret_init_next(var_name, var_val, @nospecialize(Γ::Env))
    ...
    if !(var_name in keys(Γ2.current_var_values)) # variable not initialized; use init clause
        ...
        _, Γ2 = interpret(
            AExpr(
                :assign,
                Symbol(string(:prev, uppercasefirst(string(var_name)))),
                parseautumn(
                    """(fn (state) (get (get (.. state histories) $(string(var_name)) -1) (- (.. state time) 1) $(var_name)))""",
                ),
            ),
            Γ2,
        )
    ...
    elseif Γ.state.time > 0 # variable initialized; use next clause if simulation time > 0  
        ...
        if var_val isa Array
            ...
            modified_next_func =
                sub(next_func, AExpr(:call, :prev, var_name) => unchanged_val)
            ...
            # # # # # @showΓ.state.objectsCreated
            final_val = map(
                o -> update(o, :changed, false),
                filter(obj -> obj.alive, vcat(changed_val, default_val)),
            )
        ...
        if e == true
            ...
            if Γ2.show_rules != -1
                open("likelihood_output_$(Γ2.show_rules).txt", "a") do io
                    ...
                    println(
                        io,
                        repr([event isa Symbol ? event : repr(event), repr(update_)]),
                    )
                end
            end
        ...
        if Γ2.show_rules != -1 && list != []
            open("likelihood_output_$(Γ2.show_rules).txt", "a") do io
                ...
                println(
                    ...
                    repr(
                        map(
                            x -> x isa Symbol || x isa AbstractArray ? x : repr(x),
                            [args[1], args[2][1], args[2][2], :obj, Symbol("true")],
                        ),
                    ),
                )
            end
