...
function _call_interpret(@nospecialize(Γ::Env), f, args...)
  ...
  if f == :prev && args != (:obj,)
    if length(args) == 1 
      Γ.state.time != 0 ? Γ.state.histories[args[1]][Γ.state.time - 1] : Γ.current_var_values[args[1]], Γ
    else 
      offset = interpret(args[2], Γ)[1]
      (Γ.state.time - offset) in keys(Γ.state.histories[args[1]]) ? Γ.state.histories[args[1]][Γ.state.time - offset] : 
       Γ.state.time != 0 ? Γ.state.histories[args[1]][Γ.state.time - 1] : Γ.current_var_values[args[1]], Γ
    end
  elseif islib(f)
