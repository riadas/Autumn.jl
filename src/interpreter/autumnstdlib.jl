...
mutable struct Env
    ...
    current_var_values::Dict{
        Symbol,
        Union{Object,Int,Bool,String,Position,State,AbstractArray},
    }
...
function Cell(x::Int, y::Int, color::String)
...
function Cell(x, y, color::String, @nospecialize(state::State))
    Cell(floor(Int, x), floor(Int, y), color)
end
function Cell(position::Position, color::String, @nospecialize(state::State))
    Cell(position::Position, color::String)
end
...
function clicked(
    click::Union{Click,Nothing},
    object::Object,
    @nospecialize(state::State)
)::Bool
    if isnothing(click)
        false
    else
        ...
        if GRID_SIZE isa AbstractArray
            ...
            nums = map(
                cell -> GRID_SIZE_X * cell.position.y + cell.position.x,
                render(object, state),
            )
            (GRID_SIZE_X * click.y + click.x) in nums
        else
            nums = map(
                cell -> GRID_SIZE * cell.position.y + cell.position.x,
                render(object, state),
            )
...
function clicked(
    click::Union{Click,Nothing},
    @nospecialize(objects::AbstractArray),
    @nospecialize(state::State)
)
    # # println("LOOK AT ME")
...
function pushConfiguration(
    arrow::Position,
    @nospecialize(obj1::Object),
    @nospecialize(obj2::Object),
    @nospecialize(state::State)
)
...
function pushConfiguration(
    arrow::Position,
    @nospecialize(obj1::Object),
    @nospecialize(obj2::AbstractArray),
    @nospecialize(state::State)
)
    # println("pushConfiguration: obj1 id = $(obj1.id), time = $(state.time)")
    moveIntersects(arrow, obj1, obj2, state) &&
        isFree(move(move(obj1, arrow, state), arrow, state).origin, state)
end
...
function pushConfiguration(
    ...
    @nospecialize(obj1::Object),
    @nospecialize(obj2::AbstractArray),
    @nospecialize(obj3::AbstractArray),
    @nospecialize(state::State)
)
    moveIntersects(arrow, obj1, obj2, state) &&
        intersects(move(move(obj1, arrow, state), arrow, state), obj3, state)
end
...
function intersects(
    ...
    if GRID_SIZE isa AbstractArray
        ...
        nums1 = map(
            cell -> state.histories[:GRID_SIZE][0] * cell.position.y + cell.position.x,
            render(obj1, state),
        )
        nums2 = map(
            cell -> state.histories[:GRID_SIZE][0] * cell.position.y + cell.position.x,
...
function intersects(
    ...
    if GRID_SIZE isa AbstractArray
        ...
        nums2 = map(
            cell -> GRID_SIZE_X * cell.position.y + cell.position.x,
            vcat(map(o -> render(o, state), obj2)...),
        )
        length(intersect(nums1, nums2)) != 0
    else
        nums1 = map(
...
function intersects(
    @nospecialize(obj1::AbstractArray),
    @nospecialize(obj2::AbstractArray),
    @nospecialize(state::State)
)::Bool
    ...
    elseif (obj1[1] isa Object) && (obj2[1] isa Object)
        ...
        if GRID_SIZE isa AbstractArray
            ...
            nums1 = map(
                cell -> GRID_SIZE_X * cell.position.y + cell.position.x,
                vcat(map(o -> render(o, state), obj1)...),
            )
            nums2 = map(
                cell -> GRID_SIZE_X * cell.position.y + cell.position.x,
                vcat(map(o -> render(o, state), obj2)...),
            ...
            nums1 = map(
                cell ->
                    state.histories[:GRID_SIZE][0] * cell.position.y + cell.position.x,
                vcat(map(o -> render(o, state), obj1)...),
            )
            nums2 = map(
                cell ->
...
function addObj(
    @nospecialize(list::AbstractArray),
    obj::Object,
    state::Union{State,Nothing} = nothing,
)
...
function addObj(
    @nospecialize(list::AbstractArray),
    @nospecialize(objs::AbstractArray),
    state::Union{State,Nothing} = nothing,
)
...
function updateObj(obj::Object, field::String, value, state::Union{State,Nothing} = nothing)
    ...
    constructor_values =
        map(x -> x == Symbol(field) ? value : getproperty(obj, x), constructor_fields)
...
function adjacent(
    ...
    displacement(position1, position2) in [
        Position(0, unitSize),
        Position(unitSize, 0),
        Position(0, -unitSize),
        Position(-unitSize, 0),
...
function adjacentObjs(obj::Object, unitSize::Int, @nospecialize(state::State))
    filter(
        o -> adjacent(o.origin, obj.origin, unitSize) && (obj.id != o.id),
        state.scene.objects,
    )
end
...
function adjCorner(
    ...
    filter(
        o -> o.id == obj2.id,
        map(
            obj -> !(obj.id in map(z -> z.id, adjacentObjs(obj1, unitSize, state))),
            adjacentObjsDiag(obj1, unitSize, state),
        ),
    ) != []
end
...
function adjCorner(
    ...
    obj1_adjacentObjs = vcat(
        map(
            x -> map(
                obj -> !(obj.id in map(z -> z.id, adjacentObjs(obj1, unitSize, state))),
                adjacentObjsDiag(x, unitSize, state),
            ),
            obj1,
        )...,
    )
...
function rotate(object::Object, state::Union{State,Nothing} = nothing)::Object
    ...
    new_object = update_nt(
        new_object,
        :render,
        map(x -> Cell(rotate(x.position), x.color), new_object.render),
    )
    new_object
end
...
function rotateNoCollision(object::Object, @nospecialize(state::State))::Object
    if (isWithinBounds(rotate(object), state) && isFree(rotate(object), object, state))
        rotate(object)
    else
        object
    end
...
function moveNoCollision(
    ...
    if (
        isWithinBounds(move(object, position), state) &&
        isFree(move(object, position.x, position.y), object, state)
    )
...
function moveNoCollision(object::Object, x::Int, y::Int, @nospecialize(state::State))
    if (
        isWithinBounds(move(object, x, y), state) &&
        isFree(move(object, x, y), object, state)
    )
        move(object, x, y)
    else
        object
...
function moveNoCollisionColor(
    ...
    matching_color_objects = filter(
        obj ->
            intersects(new_object, obj, state) &&
                (color in map(cell -> cell.color, render(obj, state))),
        state.scene.objects,
    )
...
function moveLeftNoCollision(object::Object, @nospecialize(state::State))::Object
    if (
        isWithinBounds(move(object, -1, 0), state) &&
        isFree(move(object, -1, 0), object, state)
    )
        move(object, -1, 0)
    else
        object
...
function moveUpNoCollision(object::Object, @nospecialize(state::State))::Object
    if (
        isWithinBounds(move(object, 0, -1), state) &&
        isFree(move(object, 0, -1), object, state)
    )
        move(object, 0, -1)
    else
        object
...
function moveWrap(object::Object, position::Position, @nospecialize(state::State))::Object
    ...
    new_object = update_nt(
        new_object,
        :origin,
        moveWrap(object.origin, position.x, position.y, state),
    )
    new_object
end
...
function moveWrap(position::Position, x::Int, y::Int, @nospecialize(state::State))::Position
    ...
    if GRID_SIZE isa AbstractArray
        ...
        Position(
            (position.x + x + GRID_SIZE_X) % GRID_SIZE_X,
            (position.y + y + GRID_SIZE_Y) % GRID_SIZE_Y,
        )
    else
        Position(
            (position.x + x + GRID_SIZE) % GRID_SIZE,
            (position.y + y + GRID_SIZE) % GRID_SIZE,
        )
    end
end
...
function distance(
    @nospecialize(objects1::AbstractArray),
    @nospecialize(objects2::AbstractArray),
    state::Union{State,Nothing} = nothing,
)::Int
...
function firstWithDefault(
    @nospecialize(arr::AbstractArray),
    state::Union{State,Nothing} = nothing,
)
    if arr == []
        Position(-30, -30)
...
function farthestRandom(
    object::Object,
    @nospecialize(types::AbstractArray),
    unit_size::Int,
    @nospecialize(state::State)
)::Position
    choices = [
        farthestLeft(object, types, unit_size, state),
...
function farthestLeft(
    ...
    if orig_position == Position(unit_size, 0)
        ...
        objects_of_type =
            filter(obj -> (obj.type in types) && (obj.alive), state.scene.objects)
        if length(objects_of_type) == 0
            Position(0, 0)
        else
            min_distance = min(map(obj -> distance(object, obj), objects_of_type))
            objects_of_min_distance =
                filter(obj -> distance(object, obj) == min_distance, objects_of_type)
...
function closestRandom(
    ...
    choices = [
        closestLeft(object, types, unit_size, state),
...
function closestLeft(
    ...
    if length(objects_of_type) == 0
        ...
        objects_of_min_distance =
            filter(obj -> distance(object, obj) == min_distance, objects_of_type)
        negative_x_displacements = filter(
            x -> x < 0,
            map(o -> (o.origin.x - object.origin.x), objects_of_min_distance),
        )
...
function closestRight(
    ...
    if length(objects_of_type) == 0
        ...
        objects_of_min_distance =
            filter(obj -> distance(object, obj) == min_distance, objects_of_type)
        positive_x_displacements = filter(
            x -> x > 0,
            map(o -> (o.origin.x - object.origin.x), objects_of_min_distance),
        )
...
function closestUp(
    ...
    if length(objects_of_type) == 0
        ...
        objects_of_min_distance =
            filter(obj -> distance(object, obj) == min_distance, objects_of_type)
        negative_y_displacements = filter(
            x -> x < 0,
            map(o -> (o.origin.y - object.origin.y), objects_of_min_distance),
        )
...
function closestDown(
    ...
    if length(objects_of_type) == 0
        ...
        objects_of_min_distance =
            filter(obj -> distance(object, obj) == min_distance, objects_of_type)
        positive_y_displacements = filter(
            x -> x > 0,
            map(o -> (o.origin.y - object.origin.y), objects_of_min_distance),
        )
...
function nextLiquid(object::Object, @nospecialize(state::State))::Object
    ...
    new_object = deepcopy(object)
    if object.origin.y != GRID_SIZE_Y - 1 &&
       isFree(move(object.origin, Position(0, 1)), state)
        new_object = update_nt(new_object, :origin, move(object.origin, Position(0, 1)))
    else
        leftHoles = filter(
            pos ->
                (pos.y == object.origin.y + 1) &&
        ...
        if (length(leftHoles) != 0) || (length(rightHoles) != 0)
            if (length(leftHoles) == 0)
                ...
                if isFree(
                    move(closestHole, Position(0, -1)),
                    move(object.origin, Position(1, 0)),
                    state,
                )
                    new_object = update_nt(
                        new_object,
                        ...
                        move(
                            object.origin,
                            unitVector(object, move(closestHole, Position(0, -1)), state),
                            state,
                        ),
                    )
                end
            elseif (length(rightHoles) == 0)
                ...
                if distance(object.origin, closestLeftHole) >
                   distance(object.origin, closestRightHole)
                    if isFree(
                        move(object.origin, Position(1, 0)),
                        move(closestRightHole, Position(0, -1)),
                        state,
                    )
                        new_object = update_nt(
                            new_object,
                    ...
                    elseif isFree(
                        move(closestLeftHole, Position(0, -1)),
                        move(object.origin, Position(-1, 0)),
                        state,
                    )
                        new_object = update_nt(
...
function nextSolid(object::Object, @nospecialize(state::State))::Object
    ...
    new_object = deepcopy(object)
    if (
        isWithinBounds(move(object, Position(0, 1)), state) && reduce(
            &,
            map(
                x -> isFree(x, object, state),
                map(cell -> move(cell.position, Position(0, 1)), render(object, state)),
            ),
        )
    )
        new_object = update_nt(new_object, :origin, move(object.origin, Position(0, 1)))
...
function isFree(start::Position, stop::Position, @nospecialize(state::State))::Bool
    ...
    reduce(
        ...
        map(
            num ->
                isFree(Position(num % GRID_SIZE_X, floor(Int, num / GRID_SIZE_X)), state),
            nums,
        ),
    )
end
...
function isFree(position::Position, object::Object, @nospecialize(state::State))
    ...
    length(
        filter(
            ...
            renderScene(
                Scene(
                    filter(obj -> obj.id != object.id, state.scene.objects),
...
function isFree(
    object::Object,
    @nospecialize(orig_object::Object),
    @nospecialize(state::State)
)::Bool
    reduce(
        &,
        map(
            x -> isFree(x, orig_object, state),
            map(cell -> cell.position, render(object, state)),
        ),
    )
end
